...
const handleOpen=(id:selectedId)=>setModalOpen(id)
          ...
          {getAllUsers.slice(1,8).map((value,index)=>(
           ...
           {userKey.slice(1,5).map((keys,keyIndex)=>(
            <td style={{padding:"15px",textAlign:"center",borderBottom:"1px  solid #E4E4E7",background:index%2===0?"white":"#F4F4F5"}}>{value[keys]}</td>
             ...
             <td style={{padding:"15px",textAlign:"center",borderBottom:"1px  solid #E4E4E7",background:index%2===0?"white":"#F4F4F5"}}> <button className={style.button} onClick={()=>handleOpen(value._id)}>Delete</button></td>    
          </tr>
          ))}
        </tbody>
      </table>
     { modalOpen&&<div style={{background:"white",boxShadow:" rgba(0, 0, 0, 0.16) 0px 1px 4px",width:"380px",height:"30%",position:"absolute",transform:'translate(50%,50%)',left:"30%",top:"30%",borderRadius:"8px"}}>
<h3 style={{textAlign:"center",padding:"10px"}}>Confirmation</h3>
<p style={{textAlign:"center",padding:"15px"}}>Are you sure want to delete this user ?</p>
<div style={{display:"flex",justifyContent:"space-evenly",padding:"15px"}}>
<button className={style.button} onClick={handleClose}>Close</button>
<button className={style.button} onClick={deleteSelectedUser}>Delete</button>
</div>
      </div>}
